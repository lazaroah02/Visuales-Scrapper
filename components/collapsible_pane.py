...
class CollapsiblePane(ttk.Frame):
    ...
    def __init__(self, parent, update_scrollregion, title = "", elements_text_list = []):
        ...
        self.button_toggle_pane.grid(row=0, column=1)
        
        # Label to show the Title
        self.label_title = ttk.Label(self, text = title)
        self.label_title.grid(row=0, column=2)
        ...
        for element in elements_text_list:
            self.elements_list.append(
                Selectable(self.content_container, text=element).grid(row=cont, column=0)
                )
            cont += 1
    ...
    def _activate(self):
        ...
        if not self.collapsed:
            ...
            self.content_container.grid(row=1, column=0, columnspan=2)
